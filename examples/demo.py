...
examples/demo.py
================
Demo end-to-end de varlib con datos REALES (descarga AAPL con DataLoader).

A partir de la descarga, el flujo es idéntico al de demo_offline.py (versión
reproducible y sin red); lo único que cambia es el principio, la obtención de
los datos.
...
# ── Datos (LO ÚNICO que difiere de demo_offline.py) ─────────────────────────────
print("=" * 60)
print("DEMO varlib - Value at Risk (AAPL, datos reales)")
print("=" * 60)

LABEL, PREFIX = "AAPL", "aapl"
print(f"\nDescargando datos de {LABEL}...", flush=True)
loader = DataLoader(LABEL, start="2020-01-01", end="2024-12-31")
loader.download()
returns = loader.get_log_returns()
train, test = loader.split(WINDOW)
